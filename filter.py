...
tot_ham_classifications = 0
num_ham_errors = 0
num_spam_errors = 0
for email in (test_set / "ham").iterdir():
    total_num_classifications += 1
    num_ham_errors += 1 - classifier.classify(open(str(email), "r", encoding="latin-1"))

tot_ham_classifications = total_num_classifications
for email in (test_set / "spam").iterdir():
    total_num_classifications += 1
    num_spam_errors += classifier.classify(open(str(email), "r", encoding="latin-1"))
